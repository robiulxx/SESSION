...
async def sign_in_pyrogram(user_id):
    ...
    try:
        ...
        # FIX: Sending code block without explicit parse_mode to avoid Invalid parse mode error
...
async def check_pyrogram_password(user_id):
    ...
    try:
        ...
        session_str = await client.export_session_string()
        # FIX: Sending code block without explicit parse_mode
        await bot.send_message(user_id,f"✅ Session তৈরি হলো:\n\n```python\n{session_str}\n```")
    except Exception as e:
...
if __name__ == "__main__":
    ...
    # This must run in a thread to allow bot.run() (which starts asyncio) to work
