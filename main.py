...
async def send_code_and_ask_for_otp(client, user_id):
    ...
    if session_type == "pyrogram":
        ...
        try:
            ...
            await client.send_message(user_id, f"কোড পাঠাতে ত্রুটি: {e}", parse_mode=None)
            cleanup_user(user_id)
...
async def sign_in_and_generate_session(client, user_id):
    ...
    if session_type == "pyrogram":
        try:
            ...
            await client.send_message(user_id, f"সাইন ইন ত্রুটি: {e}", parse_mode=None)
        finally:
...
async def check_password_and_generate_session(client, user_id):
    ...
    if session_type == "pyrogram":
        try:
            ...
            await client.send_message(user_id, "ভুল পাসওয়ার্ড! প্রক্রিয়া বাতিল।", parse_mode=None)
        except Exception as e:
            await client.send_message(user_id, f"পাসওয়ার্ড যাচাই ত্রুটি: {e}", parse_mode=None)
        finally:
            await temp_client.disconnect()
            cleanup_user(user_id)
