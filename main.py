...
try:
    bot = Client("webhook_bot", bot_token=BOT_TOKEN, api_id=int(API_ID), api_hash=API_HASH)
except Exception as e:
    # Print error but continue to allow non-pyrogram specific testing
    print(f"Error initializing Pyrogram Client: {e}")
    bot = Client("webhook_bot", bot_token=BOT_TOKEN) # Fallback
...
async def timeout_cleanup(user_id, delay=300):
    ...
    if user_id in user_steps:
        # Using simple text mode to avoid parse errors
        await bot.send_message(user_id, "OTPexpired। `/generate` দিয়ে আবার শুরু করুন।")
        cleanup_user(user_id)

অংশ ২: বট হ্যান্ডলার্স (/start, /generate, ইত্যাদি)
এই অংশটি আপনার মূল লজিক এবং ইনপুট হ্যান্ডলিং নিয়ন্ত্রণ করে।
# ----------------------
# Bot Handlers
# ----------------------
...
async def start_handler(_, message):
    await message.reply_text(
        "হ্যালো! আমি Telegram string session generator bot।\n"
...
async def callback_handler(client, query):
    ...
    await query.message.edit_text(f"{lib_choice.capitalize()} বেছে নেওয়া হলো। এখন আপনার API\_ID দিন:")
...
অংশ ৩: Pyrogram এবং Telethon লজিক
এখানে parse_mode ত্রুটি ঠিক করা হয়েছে। সেশন স্ট্রিং পাঠানোর সময় parse_mode দেওয়া হয়নি, কারণ Pyrogram নিজেই কোড ব্লকের জন্য সঠিক মোড বেছে নিতে পারে।
# ----------------------
# Pyrogram OTP
# ----------------------
...
async def sign_in_pyrogram(user_id):
    ...
    try:
        ...
        # FIX: Removed explicit parse_mode="markdown" to avoid Invalid parse mode error
...
async def check_pyrogram_password(user_id):
    ...
    try:
        ...
        session_str = await client.export_session_string()
        # FIX: Removed explicit parse_mode="markdown"
        await bot.send_message(user_id,f"✅ Session তৈরি হলো:\n\n```python\n{session_str}\n```")
    except Exception as e:
...
অংশ ৪: ওয়েবহুক সেটআপ এবং রানিং লজিক
এখানে requests ব্যবহার করে সিঙ্ক্রোনাস ওয়েবহুক সেটআপ এবং ফ্ল্যাস্ক সার্ভার শুরুর লজিকটি আছে।
# ----------------------
...
if __name__ == "__main__":
    ...
    # Note: Flask must be run in a thread to allow bot.run() (which starts asyncio) to work
    Thread(target=lambda: app.run(host="0.0.0.0", port=int(os.environ.get("PORT", 5000)))).start()
    
    # 3. Run Pyrogram Bot (Starts the main Asyncio loop)
    # The webhook updates will be received by Flask and passed to this loop.
