import os
import asyncio
import requests # <-- নিশ্চিত করুন এটি ইনস্টল করা আছে
...
async def timeout_cleanup(user_id, delay=300):
    ...
    # Check if the user is still waiting for a step before sending message
    if user_id in user_steps:
        await bot.send_message(user_id, "OTP expired। `/generate` দিয়ে আবার শুরু করুন।")
        cleanup_user(user_id)

# ----------------------
# Bot Handlers (Keep the original logic)
# ----------------------
...
def webhook():
    ...
    if update:
        ...
        # Note: asyncio.run() works here because nest_asyncio is applied
...
def set_webhook_sync(url):
    """Uses the Telegram Bot API to set the webhook, as Pyrogram Client does not have this method."""
...
if __name__ == "__main__":
    ...
    if url:
        ...
        print("WARNING: RENDER_EXTERNAL_URL environment variable is not set. Webhook will not be configured. Ensure your platform sets this variable.")

    # 2. Start the Flask server thread
    # Use a try-except block to handle PORT environment variable gracefully
    try:
